...
import { initialContacts } from '../data/HostelsAndResidentialData'; 

// --- HELPER: Hostel & Residential Icons ---
const getHostelStyle = (unitName) => {
  const name = unitName.toLowerCase();
  
  // 1. Housekeeping (Cleaning)
  if (name.includes('housekeeping')) return { icon: 'sparkles-outline', color: '#009688' }; // Teal
  
  // 2. Study Staff (Academics in Hostel)
  if (name.includes('study staff')) return { icon: 'book-outline', color: '#673AB7' }; // Deep Purple
  
  // 3. Maintenance & Bus
  if (name.includes('bus') || name.includes('maintain')) return { icon: 'bus-outline', color: '#F44336' }; // Red
  
  // 4. Guest House
  if (name.includes('guest')) return { icon: 'key-outline', color: '#FF9800' }; // Orange
  
  // 5. Hostels (General Wardens) - Check GH (Girls) vs BH (Boys)
  if (name.startsWith('gh') || name.includes('girls')) return { icon: 'bed-outline', color: '#E91E63' }; // Pink
  if (name.startsWith('bh') || name.includes('boys')) return { icon: 'bed-outline', color: '#3F51B5' }; // Indigo

  // Default fallback
  return { icon: 'home-outline', color: '#607D8B' };
};

const HostelsAndResidentialScreen = () => {
  ...
  // State
  // Initialize with initialContacts so data appears INSTANTLY
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const setupRealtimeSync = async () => {
        try {
          console.log("📡 Connecting to Firebase for Hostel Data...");
          ...
          unsubscribe = onSnapshot(
            ...
            (snapshot) => {
              ...
              const unitNames = [
                "BH HOUSE KEEPING",
                "Girls Hostel House Keeping",
                "GIRLS HOSTEL", // Girls Hostel Wardens
                "BOYS HOSTEL", // Boys Hostel Wardens
              ...
              setResidentialUnits(unitObjects);
            },
            (error) => {
              ...
              AsyncStorage.getItem('aditya_contacts_master').then(cached => {
                  if(cached) {
                      ...
                      const unitNames = ["BH HOUSE KEEPING", "Girls Hostel House Keeping", "GIRLS HOSTEL", "BOYS HOSTEL", "GIRLS HOSTEL STUDY STAFF", "BH study staff", "HOSTEL & BUS MAINT.", "Guest house"];
                      const unitObjects = unitNames.map(uName => {
                  ...
                  // No else needed, we initialized with initialContacts
              });
            }
          );
        } catch (error) {
  ...
  // ---------------------------------------------------------
  useEffect(() => {
    // If inside a folder, refresh the list automatically when data updates
    if (selectedUnit) {
        ...
        // If on the main screen, ensure units are populated initially
        if (residentialUnits.length === 0) {
             const unitNames = ["BH HOUSE KEEPING", "Girls Hostel House Keeping", "GIRLS HOSTEL", "BOYS HOSTEL", "GIRLS HOSTEL STUDY STAFF", "BH study staff", "HOSTEL & BUS MAINT.", "Guest house"];
             const unitObjects = unitNames.map(uName => {
                const count = masterContacts.filter(c => c.department && c.department.toLowerCase() === uName.toLowerCase()).length;
                return { name: uName, count };
             });
             setResidentialUnits(unitObjects);
        }
  ...
  const handleUnitPress = (unitName) => {
    // Filter contacts from MASTER (Merged) List
    const peopleInUnit = masterContacts.filter(c => 
  ...
  const renderUnitItem = ({ item }) => {
    const style = getHostelStyle(item.name);
    ...
    >
      ...
      {selectedUnit ? (
        ...
        <FlatList
          ...
          contentContainerStyle={styles.listContainer}
          showsVerticalScrollIndicator={false}
          ListHeaderComponent={<Text style={styles.sectionHeader}>Accommodation & Services</Text>}
        />
      )}
    </SafeAreaView>
  );
...
export default HostelsAndResidentialScreen;